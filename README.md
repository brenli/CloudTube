...
- **davfs2**: Для монтирования WebDAV (только Linux)
...
1. **Автоматический способ (рекомендуется):**
   ...
   ```
   Скрипт откроет браузер и автоматически получит токен.
...
**Важно:** OAuth токен действителен 1 год. После истечения получите новый токен.

📖 **Подробная инструкция по настройке Яндекс.Диска**: [YANDEX_DISK_SETUP.md](YANDEX_DISK_SETUP.md)
...
sudo apt install -y python3-pip ffmpeg sqlite3 git davfs2

# Настройка davfs2 для Яндекс.Диска
mkdir -p ~/.davfs2
